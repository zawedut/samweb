...
import { Calculator, DollarSign, CheckCircle, AlertTriangle, User, X, Frown, PartyPopper } from 'lucide-react';
import confetti from 'canvas-confetti';

const BillCalculator = ({ payer, participants }) => {
    ...
    const handleContributionChange = (person, amount) => {
    ...
    const calculate = () => {
        ...
        let totalCollectedFromFriends = 0;
        Object.values(contributions).forEach(amount => {
            totalCollectedFromFriends += (parseFloat(amount) || 0);
        });
                    ...
                    {participants.filter(p => p !== payer).map(p => (
                        <div key={p}>
                            ...
                            <input
                                ...
                                onChange={(e) => handleContributionChange(p, e.target.value)}
            ...
            <AnimatePresence>
              ...
              `}
                        >
                            ...
                            <h3 className={`text-4xl font-black mb-2 uppercase italic transform -rotate-2 
                ${results.isMatch ? 'text-green-400' : results.difference > 0 ? 'text-yellow-400' : 'text-red-400'}`}>
                                {results.isMatch ? "MISSION PASSED!" : results.difference > 0 ? "STONKS! 📈" : "WASTED"}
                            </h3>
